...
hints=(
  ...
  "FitGirl Inno Setup installs hit this on wine 11.x; cellar's path is to use a SteamRIP / Online-Fix pre-installed build instead. The CHANGELOG 'winemac.drv HWND lifecycle deadlock' section covers it."
  ...
  "EA AntiCheat is kernel-mode and has zero wine support. Only path is a community offline-EAAC-bypass crack (MKDEV's release via SteamRIP / Online-Fix). Online play is permanently blocked."
  ...
  "Cracked / standalone build (SteamRIP, Online-Fix) skips Rockstar Launcher entirely. Or use the No_GTAVLauncher exe replacement / ExeIntegrityBypassAgainstRGL.asi plugin alongside the retail exe."
  "Cracked AC / Far Cry builds ship a replacement uplay_r1_loader.dll (acidicoala's UplayR1Unlocker, SkidrowReloaded, CODEX). Make sure it's present in the game dir next to the exe."
  "cls plugin shim IPC is blocked on wine-on-Mac (cellar v0.2 stance). Cannot extract FitGirl repacks that use these codecs. Use a non-FitGirl source: SteamRIP / Online-Fix pre-installed builds."
